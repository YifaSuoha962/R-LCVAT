...
def get_module_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--model_type', help='which model to use? BiG2S, + HCVAE?', type=str,
                       choices=['BiG2S', 'BiG2S_HCVAE', 'S2S', 'S2S_HCVAE', 'G2_LW_CVAT', 'S2S_LW_CVAT'], default='BiG2S_HCVAE')
...
def post_setting_args(parser: argparse.ArgumentParser):
    ...
    if args.request == 'preprocess':
        ...
        if args.model_type in ['BiG2S_HCVAE', 'S2S_HCVAE', 'G2_LW_CVAT', 'S2S_LW_CVAT']:
            args.save_name += '-' + args.cvae_type + '-lat_sz' + str(args.lat_disc_size) + f'-lwz-{args.lat_z_size}'
        ...
        args.epochs = 800 if args.dataset_name in ['debug_uspto_50k', '50k_class', 'uspto_diverse', 'uspto_50k'] else 200       # 200 for BiG2S
        ...
        args.beam_size = 10 if args.dataset_name in ['uspto_50k', '50k_class', "debug_uspto_50k", "pistachio"] else 30
